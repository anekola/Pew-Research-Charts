...
function pew_chart_metabox_content() {
    ...
    <div class="custom_metabox">
	    ...
	    <select name="charttype" id="charttype">
	    	<option value="line" <? selected('line', $post_meta['charttype'])?>>Line</option>
	        <option value="bar" <? selected('bar', $post_meta['charttype'])?>>Bar</option>
	        <option value="column" <? selected('column', $post_meta['charttype'])?>>Column</option>
	        <option value="area" <? selected('area', $post_meta['charttype'])?>>Area</option>
	        <option value="pie" <? selected('pie', $post_meta['charttype'])?>>Pie</option>
	        <option value="scatter" <? selected('scatter', $post_meta['charttype'])?>>Scatter</option>
	    </select>
		</td></tr>
		<tr><td>
		<label for="chartheight" class="prfx-row-title">Chart Height</label>
		</td><td>
		<input type="text" name="chartheight" id="chartheight" value="<?=$post_meta['chartheight']?>" placeholder="Default: 400px" />
		</td></tr>
		<tr><td>
		<label for="chartsubtitle" class="prfx-row-title">Chart Subtitle</label>
		</td><td>
		<input type="text" name="chartsubtitle" id="chartsubtitle" value="<?=$post_meta['chartsubtitle']?>" />
		</td></tr>
		<tr><td>
		<label for="xaxistype">x-Axis Type</label>
		</td><td>
		<select name="xaxistype" id="xaxistype">
			<option value="linear" <? selected('linear', $post_meta['xaxistype'])?>>Linear</option>
	    	<option value="datetime" <? selected('datetime', $post_meta['xaxistype'])?>>Date/Time</option>
		</select>
		</td></tr>
		<tr><td>
	    <label for="xaxislabel">x-Axis Label</label>
		</td><td>
		<input type="text" name="xaxislabel" id="xaxislabel" value="<?=$post_meta['xaxislabel']?>" />
		</td></tr>
		<tr><td>
	    <label for="yaxislabel">y-Axis Label</label>
		</td><td>
		<input type="text" name="yaxislabel" id="yaxislabel" value="<?=$post_meta['yaxislabel']?>" />
	    </td></tr>
		<tr><td>
	    <label for="yaxismax">y-Axis Max Value</label>
		</td><td>
		<input type="text" name="yaxismax" id="yaxismax" value="<?=$post_meta['yaxismax']?>" />
	    </td></tr>
		<tr><td>
		<label for="zoomtype">Chart Zooming</label>
	    </td><td>
	    <select name="zoomtype" id="zoomtype">
			<option value="none" <? selected('none', $post_meta['zoomtype'])?>>None</option>
	    	<option value="x" <? selected('x', $post_meta['zoomtype'])?>>x-Axis Only</option>
			<option value="y" <? selected('y', $post_meta['zoomtype'])?>>y-Axis Only</option>
			<option value="xy" <? selected('xy', $post_meta['zoomtype'])?>>x &amp; y-Axis</option>
	    </select>
		</td></tr>
		<tr><td>
	    <label for="credits">Source Credits</label>
		</td><td>
		<input type="text" name="credits" id="credits" value="<?=$post_meta['credits']?>" />
		</td></tr>
		<tr><td>
	    <label for="credits_link">Source Link</label>
		</td><td>
		<input type="text" name="credits_link" id="credits_link" value="<?=$post_meta['credits_link']?>" />
		</td></tr>
		<tr><td colspan="2">
...
class ChartSettingsPage
{
    ...
    { ?>
        <select id="iframe" name="pew_charts[iframe]">
        	<option <? selected('disable', $this->options['iframe']); ?> value="disabled">Disabled by default</option>
        	<option <? selected('enable', $this->options['iframe']); ?> value="enabled">Enabled by default</option>
        </select>
    <? }
